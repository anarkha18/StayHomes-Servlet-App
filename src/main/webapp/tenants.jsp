    ...
    <main class="col-sm-10 overflow-auto h-100">
      <div class="bg-light border rounded-3 p-3">
        <div class="d-flex justify-content-between align-items-center">
          <h3>Tenants</h3>
          <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#tenants">
            <i class="bi bi-plus-lg"></i> Add Tenants
          </button>
        </div>
        ...
        <table class="table table-striped table-hover">
          ...
          <tbody>
          <% List<users> tenantList = (List<users>) request.getAttribute("tenantsList"); %>
          <% for (users Tenant : tenantList) { %>
          <tr>
            <th scope="row"><%= Tenant.getId() %></th>
            <td><%= Tenant.getUserName() %></td>
            <td><%= Tenant.getUserEmail() %></td>
            <td><%= Tenant.getUserPhone() %></td>
            <td>
              <a href="editUser?id=<%= Tenant.getId() %>"> <button type="button" class="btn btn-outline-primary me-2">Edit</button></a>
              <a href="deleteUser?id=<%= Tenant.getId() %>"> <button type="button" class="btn btn-primary">Delete</button></a>
            </td>
          </tr>
          <% } %>
          </tbody>
...
<!-- Modal for add user-->
<div class="modal fade" id="tenants" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">ADD TENANTS</h5>
...
<script>
  (function () {
    ...
    var tenants = document.querySelector('#tenants .needs-validation');

    // Add event listener for add user form submission
    tenants.addEventListener('submit', function (event) {
      if (!tenants.checkValidity()) {
        event.preventDefault();
        event.stopPropagation();
      }

      tenants.classList.add('was-validated');
    }, false);
  })();
</script>
